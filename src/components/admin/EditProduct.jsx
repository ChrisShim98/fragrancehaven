...
import { PutEditProduct } from "../../api/productRequests";
import { addProductForm } from "../../constants/adminFormFields";
import { FaRegWindowClose } from "react-icons/fa";

const EditProduct = ({ product, setModalOpened = () => {}, getAllProducts = () => {} }) => {
  ...
  return (
    <div className="fixed w-screen h-screen grid place-items-center z-30 top-0 left-0 bg-[#000000a5] overflow-y-scroll">
      <div className="flex flex-col gap-4 items-start lg:items-center bg-gray-200 relative my-10 p-6 rounded-lg">
        <div onClick={() => {setModalOpened(false)}} className="absolute top-[1rem] right-[1rem] text-2xl hover:cursor-pointer hover:text-red-500">
          <FaRegWindowClose />
        </div>
        <h1 className="font-medium text-xl sm:text-3xl pb-2 pt-4">Edit Product</h1>
              ...
              addProductForm.map((field) => {
                return (
                  <FormField
                    ...
                    value={field.title === "brandName" ? product.brand.name : product[field.title]}
